...
export default class WalletProvider extends Component {
    ...
    updateBalance = (amount, change, symbol) => {
        ...
        for (var i = 0; i < temp.length; i++) {
            console.log(JSON.stringify(temp[i].name))
            if (temp[i].sym === symbol) {
                index = i;
                break;
            }
        }
    ...
    addTransaction = (transaction) => {
        ...
        const { amount, convertedValue, toAddress, description, type } = transaction;
        let sym = '';

        if("selectedItem" in transaction) {
            sym = transaction.selectedItem.sym;
        }
